...
const CandidateExam = () => {
  ...
  const stopCamera = useCallback(() => {
    ...
    // 2. Stop stream directly from video element (fallback)
  ...
  const terminateExamSession = useCallback(
    async (reason) => {
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        if (!isMounted) {
          // Component unmounted while getting stream, stop it immediately
  ...
  useEffect(() => {
    ...
    const end =
      new Date(examState.startedAt).getTime() + test.duration * 60 * 1000;
  ...
  const handleSubmit = async (auto = false) => {
    ...
    try {
      ...
      // Explicitly stop all tracks and clear refs
      stopCamera();
  ...
  useEffect(() => {
    ...
    const val =
      saved?.code ??
      currentQuestion.coding?.boilerplateCode?.[lang] ??
      DEFAULT_BOILERPLATES[lang] ??
      "";
    setLanguage(lang);
    setCode(val);
    ...
    return (
      <div className="min-h-screen bg-red-950 flex items-center justify-center p-6 text-center">
        <div className="bg-slate-900 border-2 border-red-600 rounded-2xl p-10 max-w-lg w-full shadow-2xl space-y-6">
          ...
          <Button className="w-full" onClick={() => navigate("/thank-you")}>
            Go to Feedback Page
          </Button>
        </div>
      </div>
    ...
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-200 overflow-hidden relative">
      ...
      {!isFullscreen && !isSubmitted && !isTerminated && (
        <div className="absolute inset-0 z-50 bg-slate-950/95 backdrop-blur-md flex flex-col items-center justify-center text-center p-8">
          ...
          <p className="text-slate-400 max-w-md mb-8">
            Access blocked. You must be in fullscreen mode to continue. Staying
            in this mode is a violation.
          </p>
          <Button
            size="lg"
      ...
      <div className="lg:hidden fixed inset-0 z-[60] bg-slate-950 flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-slate-400 mt-2">
          This exam requires a desktop environment.
        </p>
      </div>
      ...
      <header className="h-14 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-6">
          <span
            className={`font-mono text-xl font-bold ${timeLeft < 300 ? "text-red-500 animate-pulse" : "text-white"}`}
          >
              ...
              currentQuestion.mcq.options.map((opt, i) => (
                <label
                  ...
                  className={`flex items-center p-4 border rounded-xl cursor-pointer ${
                    answers[currentQuestion._id]?.mcqAnswer === i
                ...
                <div className="flex justify-between items-center">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const saved = answers[currentQuestion._id]?.codingAnswer;
                      const val =
                        saved?.language === newLang && saved?.code
                          ? saved.code
                          : currentQuestion.coding?.boilerplateCode?.[
                              newLang
                            ] ||
                  ...
                  >
                    {(test.supportedLanguages?.length
                ...
                {answers[currentQuestion._id]?.codingAnswer && (
                  <div className="mt-4 p-4 rounded-xl bg-slate-900 border border-slate-800 text-sm space-y-1">
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-bold ${answers[currentQuestion._id].codingAnswer.verdict === "Accepted" ? "text-green-400" : "text-red-400"}`}
                      >
